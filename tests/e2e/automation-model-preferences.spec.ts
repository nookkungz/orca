...
test('edits and clears saved automation model preferences in a hidden renderer', async ({
  ...
  await orcaPage.getByText('Personal model check', { exact: true }).first().click()
  ...
  await dialog.getByLabel('Automation effort').click()
  await orcaPage.getByRole('option', { name: 'High', exact: true }).click()
  await dialog.getByRole('button', { name: /save/i }).click()
  await expect(orcaPage.getByText('gpt-5.6-terra · high', { exact: true })).toBeVisible()
  await orcaPage.getByRole('button', { name: 'Edit automation', exact: true }).click()
  await dialog.getByLabel('Automation model').click()
  await orcaPage.getByRole('option', { name: 'Use agent default', exact: true }).click()
